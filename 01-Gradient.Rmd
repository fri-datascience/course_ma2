...
$$||\nabla f|| = \sqrt{(2 + 2e^{2x})^2 + (2y)^2} \leq L on D,$$
  
where $L$ is some constant, it follows that $f$ is Lipshitz.
...
is quadratic. Its derivative is $5800x - 548$ and $f$ is minimized at $\gamma^* = \frac{548}{5800} = \frac{137}{1450}$. Pluggin this into $g(\gamma)$, we get $\frac{81}{725}$ - the minimum function value that can be achieved with a single step of gradient descent.

The global minimum of $f$ is 0 at $x^* = (1, -1)$. 
